...
if [ "$status" -ne 0 ]; then
    echo "unexpected exit code for post-add modification status: got $status want 0" >&2
    cat "$stderr_file" >&2
    exit 1
fi

if [ -s "$stderr_file" ]; then
    echo "unexpected stderr for post-add modification status" >&2
    cat "$stderr_file" >&2
    exit 1
fi

grep -F "Changes not staged for commit:" "$stdout_file" >/dev/null
grep -F "  modified:   main.txt" "$stdout_file" >/dev/null
